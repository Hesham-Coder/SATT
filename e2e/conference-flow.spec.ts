...
test("dashboard conference create flow appears on frontend", async ({ page }) => {
  ...
  await page.getByLabel("البريد الإلكتروني").fill(process.env.E2E_ADMIN_EMAIL || "admin@satt.org");
  await page.getByLabel("كلمة المرور").fill(process.env.E2E_ADMIN_PASSWORD || "admin");
