...
async function restoreBackup(backupPath) {
  try {
    ...
    try {
      // Safe per-entry extraction — reject path traversal, absolute paths, and symlinks
      ...
      for (const entry of entries) {
        ...
        if (normalized.includes('..') || path.isAbsolute(rawName)) {
          logger.warn('Backup restore: rejected unsafe entry path', { entry: rawName });
          continue;
        }
        ...
        if (unixFileType === 0xA000) {
          logger.warn('Backup restore: rejected symlink entry', { entry: rawName });
          continue;
        }

        const destPath = path.resolve(path.join(tempDir, normalized));
        // Belt-and-suspenders: ensure destination is still inside tempDir
        if (!destPath.startsWith(resolvedTemp + path.sep)) {
          logger.warn('Backup restore: rejected entry escaping temp dir', { entry: rawName });
          continue;
        }
