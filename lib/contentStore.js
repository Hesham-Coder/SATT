...
/**
 * Extract a plain phone string from either a string or a legacy bilingual object
 * {en, ar} that older editor versions may have stored, and strip invisible
 * Unicode bidi control characters injected by RTL helpers.
 */
function extractPhoneString(val, fallback) {
  const DEFAULT_FALLBACK = '+201120800011';
  let s;
...
function ensureContactSettings(content) {
  ...
  // Extract legacy contact phone strings, handling bilingual objects from older data formats.
  const legacyPhone = extractPhoneString(content.contact.phone, '');
  const legacyEmergency = extractPhoneString(content.contact.emergencyPhone, '');

  // Normalise each phone field: convert bilingual objects and strip invisible chars.
  const DEFAULT_PHONE = '+201120800011';
  content.contactSettings.primaryNavbarNumber =
    extractPhoneString(content.contactSettings.primaryNavbarNumber, legacyPhone || DEFAULT_PHONE);
  content.contactSettings.immediateSupportNumber =
    extractPhoneString(content.contactSettings.immediateSupportNumber,
      legacyEmergency || content.contactSettings.primaryNavbarNumber);
  content.contactSettings.footerGeneralContact =
    extractPhoneString(content.contactSettings.footerGeneralContact,
      legacyPhone || content.contactSettings.primaryNavbarNumber);
  content.contactSettings.whatsappSupportNumber =
    extractPhoneString(content.contactSettings.whatsappSupportNumber,
      content.contactSettings.primaryNavbarNumber);
...
function ensureExperts(content) {
  ...
  if (!Array.isArray(content.sectionsOrder)) content.sectionsOrder = ['hero', 'hero2', 'hero3', 'services', 'team', 'testimonials', 'about', 'certificates', 'contact', 'cta'];
  ['hero2', 'hero3'].forEach((sectionId) => {
    if (!content.sectionsOrder.includes(sectionId)) {
      const heroIndex = content.sectionsOrder.indexOf('hero');
      if (heroIndex >= 0) content.sectionsOrder.splice(heroIndex + 1, 0, sectionId);
      else content.sectionsOrder.unshift(sectionId);
    }
  });
  ...
  if (content.sectionVisibility.hero2 === undefined) content.sectionVisibility.hero2 = true;
  if (content.sectionVisibility.hero3 === undefined) content.sectionVisibility.hero3 = true;
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (err.code !== 'ENOENT') {
          logger.warn('Failed to parse users.json, will attempt to repair', { error: err.message });
        }
        if (!ADMIN_BOOTSTRAP_PASSWORD) {
          logger.warn('ADMIN_BOOTSTRAP_PASSWORD not set and users.json is missing; skipping admin creation');
          return;
        }
      }

      if (ADMIN_BOOTSTRAP_PASSWORD) {
        // Use 10 rounds for bcryptjs for better performance while maintaining security
        const hashedPassword = await bcrypt.hash(ADMIN_BOOTSTRAP_PASSWORD, 10);
        const now = new Date().toISOString();
        
        if (users[username]) {
          users[username].password = hashedPassword;
          users[username].email = ADMIN_BOOTSTRAP_EMAIL || users[username].email || '';
          users[username].updatedAt = now;
          logger.info('Admin user credentials synchronized from environment variables');
        } else {
          users[username] = {
            username,
          ...
          logger.info(fileExists ? 'Admin user created (synchronized from env)' : 'Users file created using bootstrap environment credentials');
        }
        
        await fs.writeFile(USERS_FILE, JSON.stringify(users, null, 2));
      }
    } catch (error) {
      logger.error('Critical error during admin user synchronization', { error: error.message });
    }
